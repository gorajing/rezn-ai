...
def _harmony_notes(key: str, mode: str, degree: int, start: float, energy: float, style: Style) -> list[Note]:
    chord = _chord(key, mode, degree, style.harmony_octave, style.harmony_voices)
    velocity = _vel((64 + energy * 28 * style.dynamics) * style.harmony_gain)
    return [Note("harmony", pitch, start, 4.0, velocity) for pitch in chord]


def _bass_notes(key: str, mode: str, degree: int, start: float, energy: float, style: Style) -> list[Note]:
    velocity = _vel((72 + energy * 24 * style.dynamics) * style.bass_gain)
...
def _drum_notes(start: float, energy: float, phrase_bar: int, style: Style) -> list[Note]:
...
def _texture_notes(key: str, mode: str, degree: int, start: float, energy: float, style: Style) -> list[Note]:
    ...
    step_dur = 4.0 / steps
    velocity = _vel((50 + energy * 34 * style.dynamics) * style.texture_gain)
    return [
        Note(
            "texture",
            order[i % len(order)],
...
def compose_arrangement(
    *, title: str, key: str, mode: str, tempo: float, seed: int, strategy: str = "default"
) -> dict[str, Any]:
    ...
    for section, section_start in zip(DEFAULT_FORM, starts, strict=True):
        ...
        for local_bar in range(section.bars):
            ...
            if "harmony" in section.active_parts:
                notes.extend(_harmony_notes(key, mode, degree, bar_start, section.energy, style))
            if "texture" in section.active_parts:
                notes.extend(_texture_notes(key, mode, degree, bar_start, section.energy, style))
            if "bass" in section.active_parts:
                notes.extend(_bass_notes(key, mode, degree, bar_start, section.energy, style))
            if "drums" in section.active_parts:
                notes.extend(_drum_notes(bar_start, section.energy, local_bar % 8, style))
            bar_cursor += 1
